...
- `GET /cars/{carId}/map/points?from&to&kinds=` — flat lat/lon rows for Grafana Geomap. Path SQL is chosen from the drive list: hops under ~2.5 min / 0.35 km use start/end; longer drives share an 8k-point budget and get their own sample interval. `kinds=charge|park` skip path load.
...
- `GET /cars/{carId}/map/focus?day&slot&kind&from&to&id&kinds=&timezone=&dayStartHour=` — map points for a focused trip. `kinds=park,charge` is the pin layer; drive samples stay on the route layer.
