...
- `GET /cars/{carId}/map/points?from&to&kinds=` — flat lat/lon rows for Grafana Geomap. Drive paths thin by window length (time range is the zoom). `kinds=charge|park` skip path load.
